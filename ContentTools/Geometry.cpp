...
void pack_vertices_static(mesh &m)
{
    ...
    for (u32 i{0}; i < num_vertices; ++i)
    {
        ...
        m.packed_vertices_static.emplace_back(
            packed_vertex::vertex_static{v.position, {0, 0, 0}, signs, {normal_x, normal_y}, {}, v.uv});
    }
}
...
u64 get_mesh_size(const mesh &m)
{
    ...
    const u64 size{su32 + m.name.size() + su32 + su32 + su32 + su32 + su32 + sizeof(f32) + vertex_buffer_size +
                   index_buffer_size};
