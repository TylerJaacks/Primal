                  ...
                  v2 v_range = {0.f, 1.f})
{
    ...
    for (u32 j{0}; j < vertical_count; ++j)
    {
        ...
        {
            const u32 index[4]{i + j * row_length, i + (j + 1) * row_length, (i + 1) + j * row_length,
                               (i + 1) + (j + 1) * row_length};
...
mesh create_uv_sphere(const primitive_init_info &info)
{
    ...
    for (u32 j{1}; j <= (theta_count - 1); ++j)
    {
        ...
        for (u32 i{0}; i < phi_count; ++i)
        {
            ...
            m.positions[c++] = {info.size.x * XMScalarSin(theta) * XMScalarCos(phi), info.size.y * XMScalarCos(theta),
    ...
    for (u32 j{0}; j < (theta_count - 2); ++j)
    {
        for (u32 i{0}; i < (phi_count - 1); ++i)
        {
            const u32 index[4]{1 + i + j * phi_count, 1 + i + (j + 1) * phi_count, 1 + (i + 1) + (j + 1) * phi_count,
                               1 + (i + 1) + j * phi_count};
        ...
        const u32 index[4]{phi_count + j * phi_count, phi_count + (j + 1) * phi_count, 1 + (j + 1) * phi_count,
                           1 + j * phi_count};
