...
namespace primal::graphics::d3d12::fx 
{
	...
	bool create_fx_pos_and_root_signature()
	{
		...
		// Create Post-Processing FX PSO.
		struct
		{
			d3dx::d3d12_pipeline_state_subobject_root_signature				root_signature{ fx_root_sig };
			d3dx::d3d12_pipeline_state_subobject_vs							vs{ get_engine_shader(shaders::engine_shader::fullscreen_triangle_vs) };
			d3dx::d3d12_pipeline_state_subobject_ps							ps{ get_engine_shader(shaders::engine_shader::post_processing_ps) };
		...
		NAME_D3D12_OBJECT(fx_pso, L"Post-Processing FX PSO");
	...
	void post_processing(id3d12_graphics_command_list* cmd_list, D3D12_CPU_DESCRIPTOR_HANDLE target_rtv)
	{
